...
# ╔═╡ f087c021-2ef2-4084-bcf5-7c1f53489a92
begin
	...
	function simulate(iterations)
	    ...
	    for i in 1:iterations
	        ...
	        push!(earth_velocities, earth.velocity)
			push!(forces, calc_f(earth, sun))
	        step!(all_parts)
	    end
	
	    earth_posis, earth_velocities, forces
	    # "yo"
	end
...
# ╔═╡ a92036bb-f6ea-4dae-8291-1dbc2063de10
p, v, f = simulate(700_000)

# ╔═╡ 795fd29a-44a5-437b-a656-ff5c05e15387
begin
	ps = [float(x[1]) for x in p]
	vs = [float(x[1]) for x in v]
	fs = float.(f)
end

# ╔═╡ c5a75c24-ece6-4969-98f1-9820d9bceb7b
begin
	min_p = minimum(ps)
	passing = findfirst(x->x>=0, ps)
	max_v = maximum(vs)
	max_v_idx = findfirst(x->x==max_v, vs)
end

# ╔═╡ 62debd8d-025a-48fd-bc9a-0706b71a2fc3
passing

# ╔═╡ a97481ef-34f1-437d-8c5f-1b42e9874c23
begin
	fig = Figure()
	ax = Axis(fig[1, 1])
	ax2 = Axis(fig[2, 1])
	ax3 = Axis(fig[3, 1])
	space = 100
	relevant_ps = ps[passing-space:passing+space]
	relevant_vs = vs[passing-space:passing+space]
	relevant_fs = fs[passing-space:passing+space]
	lines!(ax, passing-space:passing+space, relevant_ps)
	lines!(ax2, passing-space:passing+space, relevant_vs)
	lines!(ax3, passing-space:passing+space, relevant_fs)
	current_figure()
end

# ╔═╡ 06d3c132-461e-4360-a140-a3c9fe9fd145
typeof(f)
